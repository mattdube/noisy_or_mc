...
# ===========================================================================
# TEST 5 — Full batch pipeline with CSV input
# ===========================================================================
section("TEST 5: Full Batch Pipeline (CSV → Excel)")
...
check("Excel output file created",
      os.path.exists(OUTPUT_XLSX),
      OUTPUT_XLSX)
...
try:
    ...
    df_all = pd.read_excel(OUTPUT_XLSX, sheet_name="All_Records")
    check("All_Records has correct row count",
          len(df_all) == n_pipe,
          f"{len(df_all)} rows")

    check("All_Records has risk_point column",
          "risk_point" in df_all.columns)

    check("All_Records has risk_tier column",
          "risk_tier" in df_all.columns)
    ...
    df_alerts = pd.read_excel(OUTPUT_XLSX, sheet_name="High_Risk_Alerts")
    ...
    df_model = pd.read_excel(OUTPUT_XLSX, sheet_name="Model_Info")
    check("Model_Info sheet has content",
          len(df_model) > 0,
          f"{len(df_model)} rows of metadata")

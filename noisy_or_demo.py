...
Demonstrates all capabilities of NoisyORModel.
  ...
  3.  Batch prediction — optimized path (pool + vectorized + dedup)
  4.  Deduplication in action — repeated flag patterns scored once
  5.  Batch performance benchmark vs. old per-record loop
  6.  Export to Excel and CSV
  7.  Generate an updated config from a Python dictionary
...
import time
import numpy as np
from noisy_or_model import NoisyORModel, _run_simulation
...
# -----------------------------------------------------------------------
# 3.  Batch prediction — optimized path
...
df = model.predict_batch(active_matrix=batch_inputs, labels=batch_labels)
batch_ms = (time.perf_counter() - t0) * 1000

print(f"=== Batch Results ({len(batch_inputs)} records, {batch_ms:.1f} ms) ===")
display_cols = [
...
# -----------------------------------------------------------------------
# 5.  Performance benchmark — new vectorized path vs. old per-record loop
# -----------------------------------------------------------------------
...
print(df_excel[["Label", "Risk_Score", "Certainty_Label"]].to_string(index=False))

...
class NoisyORModel:
    ...
    def _build_batch_pool(self, seed: Optional[int] = None) -> None:
        ...
        # Cache: maps frozen flags tuple → raw simulation dict
    ...
    def predict_batch(
        ...
        Returns
        -------
        pandas DataFrame — same column layout as before.
        """
        ...
        for row in active_matrix:
            self._validate_flags(row)

        rng = np.random.default_rng(seed)

        # --- Deduplication: find unique flag combinations ---
        # Convert each row to a tuple key; map each record to its unique row
        keys        = [tuple(row.tolist()) for row in active_matrix]
        unique_keys = list(dict.fromkeys(keys))   # preserves insertion order
        key_to_idx  = {k: i for i, k in enumerate(unique_keys)}

        # Build unique sub-matrix for vectorized scoring
        unique_matrix = np.array(unique_keys, dtype=int)

        # Score all unique rows in one vectorized pass
        unique_sims = _run_batch_vectorized(
            self._batch_pool, self._batch_source_means,
            unique_matrix, n, rng,
        )

        # Build a lookup: unique_key → sim dict
        sim_lookup = {k: unique_sims[i] for i, k in enumerate(unique_keys)}

        # --- Assemble output in original record order ---
        rows = [
    ...
    def save_config(
        ...
        output_path  : where to write the new .json file
        version      : explicit version string (e.g. "2.1.0"); if omitted the
                       current version is auto-bumped according to `bump`
        n_samples    : Monte Carlo sample count for the new config; defaults
                       to the currently loaded value
        bump         : which part of the version to auto-increment when
                       `version` is not supplied — "major", "minor", or "patch"
        """

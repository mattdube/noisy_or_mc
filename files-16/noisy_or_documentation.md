...
The standard Noisy-OR model assumes source independence. When sources are positively correlated — both frequently fire together because they share an underlying driver — the independent model overestimates combined risk by treating two signals that carry the same information as if they were independent.
...
- You want to accurately attribute combined risk without inflating it
...
`rho` must be in the open interval (-1, 1). Positive values mean the sources tend to fire together with similar strength; negative values mean they tend to be inversely related. If the full correlation matrix is not positive semi-definite (which can happen with inconsistent manual values), the model automatically projects it to the nearest valid matrix and warns you.
...
print(model)
# NoisyORModelCopula(v1.0.0, n_sources=4, rho_pairs=2)

model.print_correlations()
# Configured pairwise correlations (2 pair(s)):
#   Source A                Source B                   rho
#   Watchlist Hit           Adverse Media             0.650
#   Watchlist Hit           PEP Match                 0.400
...
```python
correlated  = NoisyORModelCopula("noisy_or_config_correlated.json")
independent = correlated.with_independence()   # same mu/kappa, no correlations

flags = np.array([1, 1, 0, 0])   # Watchlist Hit + Adverse Media
r_corr = correlated.predict(flags)["risk_score"]
r_ind  = independent.predict(flags)["risk_score"]

print(f"Correlated  (rho=0.65): {r_corr:.1%}")
print(f"Independent (rho=0.00): {r_ind:.1%}")
# Correlated:  72.4%   ← lower, correctly accounting for shared signal
# Independent: 82.0%   ← higher, incorrectly treating signals as unrelated
```

The correlated estimate is lower because the model recognises that when both Watchlist and Adverse Media fire, they likely share an underlying driver, so the combination carries less total information than two truly independent signals would.
...
The marginal distribution of each `p_i` is still exactly `Beta(αᵢ, βᵢ)` — `mu` and `kappa` are respected — but the joint distribution of `(p₁, …, pₙ)` now respects the correlation matrix. The Noisy-OR combination then proceeds identically.

When all correlations are zero, `R` is the identity, the Cholesky factor is also the identity, and the result is mathematically identical to the standard model.
...
Step 4 — Noisy-OR combination (same as standard model):
  combined = 1 - prod(1 - p_i)  for active sources i
```
The marginal distribution of each `p_i` is exactly `Beta(alpha_i, beta_i)` regardless
of the correlation structure. When R = I (all rho = 0), step 1 is a no-op and the
result is identical to standard independent sampling.

...
def _parse_case_feedback_csv(
    ...
    """
    Parse a case-level feedback CSV and return:
      - per-source weighted counts (for the Bayesian update)
      - per-case contribution detail (for the audit log)

    Returns
    -------
    (counts, case_contributions)
      counts              : source_name → {"n_positive": float, "n_negative": float}
      case_contributions  : list of dicts, one per valid case:
                            {active_sources, outcome, weight, method,
                             per_source_credits: {source: credit}}
    """
    path   = Path(path)
    counts: dict[str, dict[str, float]] = defaultdict(
        lambda: {"n_positive": 0.0, "n_negative": 0.0}
    )
    case_contributions: list[dict] = []
    skipped = 0
    ...
    with open(path, newline="", encoding="utf-8-sig") as f:
        ...
        for row_num, row in enumerate(reader, start=2):
            ...
            per_source_credits: dict[str, float] = {}

            for source, attr_weight in attribution.items():
                effective = attr_weight * weight
                per_source_credits[source] = round(effective, 6)
                if outcome == 1:
            ...
            case_contributions.append({
                ...
                "per_source_credits":  per_source_credits,
            })

    if skipped:
        logger.warning("%d row(s) skipped in %s", skipped, path.name)

    return dict(counts), case_contributions
...
class FeedbackUpdater:
    ...
    kappa_cap           : optional upper bound on kappa after each update.
                          When kappa_new would exceed this value, alpha and
                          beta are rescaled so kappa equals the cap while mu
                          is preserved exactly.  This implements sliding-window
                          Bayesian updating — the model retains its learned mu
                          but stays responsive to future feedback rather than
                          becoming asymptotically resistant as evidence
                          accumulates.  Set to None (default) to allow kappa
                          to grow without bound.  A value of 100 is equivalent
                          to "trust at most ~100 observations of evidence."
    """
    ...
    def __init__(
        ...
        if kappa_cap is not None and kappa_cap <= 0:
            raise ValueError(f"kappa_cap must be > 0, got {kappa_cap}")
    ...
    def apply(
        ...
        fmt, counts, case_contributions = self._parse(
    ...
    def _parse(
        ...
        """
        Auto-detect feedback format and return (format_str, counts_dict, case_contributions).
        case_contributions is None for source-level (Format 1) files.
        """
    ...
    def _compute_changes(
        ...
        for name, c in counts.items():
            ...
            # Apply kappa cap — preserve mu exactly, rescale alpha/beta
            kappa_capped = False
            if self.kappa_cap is not None and kappa_after > self.kappa_cap:
                alpha_capped = mu_after * self.kappa_cap
                beta_capped  = (1.0 - mu_after) * self.kappa_cap
                mu_after, kappa_after = _mu_kappa(alpha_capped, beta_capped)
                kappa_capped = True
                logger.debug(
                    "kappa_cap applied for '%s': kappa rescaled to %.2f "
                    "(mu preserved at %.6f)", name, self.kappa_cap, mu_after
                )
    ...
    def _log_update(
        ...
        case_contributions: Optional[list] = None,
    ) -> None:
        entry = {
        ...
        # Case-level contributions — only written for Format 2 (case-level) files.
        # Each entry is one case: {active_sources, outcome, weight, per_source_credits}.
        # This makes the log fully self-contained: any historical state can be
        # recomputed from the log alone, and "what-if I remove case X?" queries
        # are answerable without re-parsing the original feedback file.
        if case_contributions is not None:
            entry["case_contributions"] = case_contributions

...
14. [Configurable Risk Bands](#configurable-risk-bands)
    - [Configuring Risk Bands](#configuring-risk-bands)
    - [Using Risk Bands at Runtime](#using-risk-bands-at-runtime)
    - [Updating Risk Bands via save_config](#updating-risk-bands-via-save_config)
15. [reload() — Standard Model Hot Reload](#reload----standard-model-hot-reload)
16. [stream_export() — Memory-Efficient Large Exports](#stream_export----memory-efficient-large-exports)
17. [kappa_cap — Preventing Model Staleness](#kappa_cap----preventing-model-staleness)
18. [Correlated Sources — Gaussian Copula Model](#correlated-sources--gaussian-copula-model)
    - [When to Use](#when-to-use)
    - [Configuration](#configuration-1)
    - [Usage](#usage-1)
    - [Two Corrections Applied Over the Standard Model](#two-corrections-applied-over-the-standard-model)
    - [Comparing Correlated vs Independent](#comparing-correlated-vs-independent)
    - [Marginal Contributions Under Correlation](#marginal-contributions-under-correlation)
    - [How the Copula Works](#how-the-copula-works)
    - [Negative Correlation](#negative-correlation)
    - [Saving Correlated Configs](#saving-correlated-configs)
19. [Config Versioning](#config-versioning)
20. [Interpreting Results](#interpreting-results)
21. [Mathematical Reference](#mathematical-reference)
...
**`noisy_or_model_copula.py` — Correlated model** extends the standard model with a Gaussian copula to account for correlated sources. When two sources frequently co-fire together (e.g. a watchlist hit and adverse media both linked to the same underlying risk), treating them as independent overestimates combined risk. The copula preserves each source's Beta marginal distribution exactly while introducing user-specified pairwise correlation coefficients.

All three variants share the same output format, the same config file structure, and the same source management workflow.
...
noisy_or_model_copula.py     <- Correlated model (NoisyORModelCopula, Gaussian copula)
noisy_or_demo_copula.py      <- Demo: correlated model walkthrough (8 sections)
...
The correlated model (`NoisyORModelCopula`) additionally requires scipy:

```bash
pip install scipy   # required for NoisyORModelCopula only
```
...
| `risk_bands` | array, optional | List of `{"label": str, "min_score": float}` objects defining named risk tiers. Scored descending — the first band whose `min_score` the record meets is used. Adds a `Risk_Band` column to all batch output. |
| `correlations` | array, optional | *NoisyORModelCopula only.* List of `{"source_a": str, "source_b": str, "rho": float}` objects specifying pairwise correlation. Only non-zero pairs need be listed. |
...
| `risk_band` | str or None | Risk band label if `risk_bands` are configured in the config, otherwise `None`. |
...
| `Risk_Band` | Risk band label (only present when `risk_bands` are configured). |
...
Risk bands segment the numeric `Risk_Score` into labelled tiers. They are defined once in the config and automatically appear as a `Risk_Band` column in all batch output — DataFrame, CSV, and Excel.

### Configuring Risk Bands

Add a `risk_bands` array to the JSON config. Each entry needs a `label` and a `min_score` threshold. The model assigns the label of the first band whose threshold the record's risk score meets or exceeds, evaluated in descending order.
...
Thresholds do not need to be exhaustive. A record scoring below the lowest threshold receives `None` / an empty string in the `Risk_Band` column.

### Using Risk Bands at Runtime

```python
model = NoisyORModel("noisy_or_config.json")

# Inspect configured bands
print(model.risk_bands)
# [(0.75, 'Critical'), (0.5, 'High'), (0.25, 'Medium'), (0.0, 'Low')]

# Risk_Band column appears automatically in batch output
...
# Customer_D      0.0000        Low
```

### Updating Risk Bands via save_config

```python
model.save_config(
    sources_dict=my_sources,
    output_path="noisy_or_config.json",
    risk_bands=[
        {"label": "Urgent",  "min_score": 0.80},
        {"label": "Review",  "min_score": 0.50},
        {"label": "Monitor", "min_score": 0.00},
    ],
)
# Pass risk_bands=[] to remove all bands from the saved config.
# If risk_bands is omitted, existing bands are preserved unchanged.
```

---

## reload() — Standard Model Hot Reload

`NoisyORModel.reload()` re-reads the config file and rebuilds the batch pool in-place, matching the behaviour of `NoisyORModelAPI.reload()`. Use this after a `FeedbackUpdater.apply()` call to pick up new parameters without reinstantiating.
...
updater.apply("feedback_march.csv")
model.reload()   # picks up new mu/kappa values immediately
print(model)     # NoisyORModel(v1.0.1, n_sources=8, n_samples=20000)
```

**Note:** unlike `NoisyORModelAPI.reload()`, the standard model's `reload()` is not thread-safe. If concurrent batch calls may be in flight during the reload, use `NoisyORModelAPI` instead.

---

## stream_export() — Memory-Efficient Large Exports

`stream_export()` processes a large batch in chunks and appends each chunk to the output CSV incrementally. Peak memory usage is proportional to `chunk_size` rather than the full dataset, making arbitrarily large exports possible.

Only CSV output is supported — Excel requires the full workbook in memory before saving. For Excel, use `export_batch()` on datasets that fit comfortably in RAM.

```python
# Standard model
model.stream_export(
    active_matrix = large_matrix,     # 500,000 records, no problem
    output_path   = "results.csv",
    chunk_size    = 10_000,           # default 5,000; tune to your RAM
)

# API model (adds precision parameter)
api_model.stream_export(
    active_matrix = large_matrix,
    output_path   = "results.csv",
    precision     = "fast",           # use fast tier to maximise throughput
    chunk_size    = 10_000,
)
```

The output file is identical to `export_batch(...).to_csv()` — the chunking is transparent. The `polars` backend is also supported for the CSV writer.

---

## kappa_cap — Preventing Model Staleness

As feedback accumulates, `kappa` grows without bound. After hundreds of observations, `kappa` becomes very large, which means new feedback has almost no effect — the model is effectively frozen even if the underlying risk signal has changed.

`kappa_cap` sets an upper bound on `kappa` after each update. When `kappa_new` would exceed the cap, `mu` is preserved exactly but `alpha` and `beta` are rescaled so that `kappa` equals the cap:

```
mu_new    = alpha_new / kappa_new        (unchanged)
kappa_new = min(kappa_new, kappa_cap)    (capped)
alpha_cap = mu_new * kappa_cap
beta_cap  = (1 - mu_new) * kappa_cap
```

This implements **sliding-window Bayesian updating**: the model retains its learned `mu` estimate but stays responsive to future feedback.
...
A cap of 100 means: regardless of how many observations have accumulated, the model will always treat new feedback as if there were only 100 prior observations behind the current `mu`. This prevents long-running models from becoming resistant to genuine distributional shift.

`kappa_cap` defaults to `None` (unbounded), preserving existing behaviour.

---

## Correlated Sources — Gaussian Copula Model

### When to Use

The standard Noisy-OR model assumes source independence. When sources are positively correlated — both frequently fire together because they share an underlying driver — the independent model overestimates combined risk by treating two signals that carry essentially the same information as if they were independent.

Use `NoisyORModelCopula` when:
- Two or more sources frequently co-fire and are known to share an underlying driver
- You have calibrated correlation estimates (from historical co-occurrence rates or domain knowledge)
- You want combined risk scores that correctly reflect near-redundant signals rather than inflating them

### Installation

```bash
pip install scipy   # required for NoisyORModelCopula
```

### Configuration

Add a `"correlations"` section to the JSON config. Only non-zero pairs need to be listed.
...
```json
{
  ...
  "sources": [
    ...
    {"name": "Geography Risk", "mu": 0.30, "kappa": 8.0}
  ],
  "correlations": [
...
`rho` must be in the open interval (-1, 1). Positive values mean the sources tend to fire together with similar strength; negative values mean they tend to be inversely related. If the full correlation matrix is not positive semi-definite (which can happen with inconsistent manual values), a `ValueError` is raised with a clear message.
...
print(model)
# NoisyORModelCopula(v1.0.0, n_sources=4, n_samples=20000, corr_pairs=2)

model.print_correlations()
# Correlation matrix (4×4):
#   ...
# Configured correlations (2):
#   Watchlist Hit        ↔ Adverse Media        ρ = +0.65  █████████████
#   Watchlist Hit        ↔ PEP Match            ρ = +0.40  ████████

# Predict — API identical to NoisyORModel
flags  = np.array([1, 1, 0, 0])   # Watchlist + Adverse Media both fired
result = model.predict(flags, label="Customer_A")
model.print_result(result)

# Batch prediction — all optimizations (pool, vectorization, chunking) apply
df = model.predict_batch(np.array([[1,1,0,0],[0,1,1,0],[1,0,0,1]]))
```

### Two Corrections Applied Over the Standard Model

The copula model applies two separate corrections that work together to properly handle correlated sources.

#### 1. Correlated Sampling

Beta probability samples are drawn *jointly* using the Gaussian copula rather than independently. This means that when Source A draws a high probability value, Source B (if highly correlated with A) also tends to draw a high value in the same simulation sample. Sources with rho=0 still draw independently.

#### 2. Redundancy-Aware Combination (Blending)

Correlated sampling alone is not sufficient. Even with correlated draws, the Noisy-OR formula still combines the samples as if they carry independent evidence, which overestimates risk when sources are nearly redundant.

The model therefore applies a **blended combination rule** that interpolates between Noisy-OR (full independence) and the maximum of the active source probabilities (full redundancy):

```
combined = (1 − ρ_eff) × noisy_or(p_i…) + ρ_eff × max(p_i…)
```

where `ρ_eff` is the mean pairwise correlation across the active sources for that record.

**Boundary behaviour:**
- `ρ_eff = 0.0` → pure Noisy-OR, identical to the standard model
- `ρ_eff → 1.0` → combined risk approaches `max(p_i)`, the strongest single source

This correctly models the intuition: when two sources are nearly perfectly correlated they are essentially the same signal seen twice. The combined risk should be close to the stronger source alone, not the Noisy-OR of two independent signals.

**Example with mu_WL=0.70, mu_AM=0.55, both active** (from the demo):

| rho | Combined risk | Reduction vs independent | Interpretation |
|-----|--------------|--------------------------|----------------|
| 0.00 | 86.5% | — | Standard Noisy-OR (fully independent) |
| 0.25 | 82.5% | −4.6% | Slight shared information |
| 0.50 | 78.5% | −9.2% | Moderate redundancy |
| 0.75 | 74.1% | −14.3% | Substantial redundancy |
| 0.95 | 70.7% | −18.3% | Near-redundant — close to max source |
| 0.99 | 70.1% | −18.9% | Effectively the same signal (≈ mu_WL) |

### Comparing Correlated vs Independent
...
# Build a correlated config and load both models
cfg = json.loads(Path("noisy_or_config.json").read_text())
cfg["correlations"] = [
    {"source_a": "Watchlist Hit", "source_b": "Adverse Media", "rho": 0.75}
]
Path("config_corr.json").write_text(json.dumps(cfg, indent=2))

correlated  = NoisyORModelCopula("config_corr.json")
independent = NoisyORModelCopula("noisy_or_config.json")   # no correlations block

flags = np.array([1, 1, 0, 0, 0])   # Watchlist Hit + Adverse Media both fired

rc = correlated.predict(flags)
ri = independent.predict(flags)

print(f"Independent (rho=0.00):  {ri['risk_score']:.1%}")
print(f"Correlated  (rho=0.75):  {rc['risk_score']:.1%}")
# Independent: 86.5%
# Correlated:  74.1%
```

Both the risk score and the marginal contributions are lower under high correlation. This is intentional and correct — when Adverse Media is already matched, Watchlist Hit adds less incremental evidence because the two sources are largely redundant at rho=0.75.

**Important:** the correlated model's `predict()` method draws fresh correlated samples via the Gaussian copula on every call. This is different from `predict_batch()`, which draws from a pre-built correlated pool. Both paths apply the blended combination rule — the results will agree statistically across many calls, though individual calls will differ due to Monte Carlo variance.

### Marginal Contributions Under Correlation

Marginal contributions are computed consistently with the blended combination rule:

```
c_i = R_blended(all active) − R_blended(all active except i)
```

where `R_blended` applies the same Noisy-OR ↔ max interpolation. This means contributions correctly shrink as correlation increases. Using the demo scenario (Watchlist Hit mu=0.70, Adverse Media mu=0.55, both active):

| Scenario | Risk score | Contrib WL | Contrib AM |
|---|---|---|---|
| rho=0.00 (independent) | 86.5% | 31.4% | 16.5% |
| rho=0.75 (substantial redundancy) | 74.1% | 19.1% | 4.1% |

At rho=0.75: Adverse Media is already matched, so Watchlist Hit adds only ~19% incremental risk (down from ~31%). Adverse Media adds only ~4% on top of Watchlist Hit (down from ~16%). The more redundant the sources, the smaller each one's individual contribution.

This is in deliberate contrast to the standard model, where contributions are always computed from the Noisy-OR formula regardless of correlation — so the standard model would report the same contributions whether rho is 0 or 0.75.

**Three correlated sources:** when three sources all co-fire, `ρ_eff` is the mean of all three pairwise correlations. In the demo (WL+AM+PEP, rho_eff ≈ 0.567), each source's contribution shrinks significantly compared to the independent model:

| Source | Independent | Correlated (rho_eff≈0.57) | Reduction |
|---|---|---|---|
| Watchlist Hit | 12.6% | 7.5% | −40% |
| Adverse Media | 6.6% | 2.6% | −61% |
| PEP Match | 8.1% | 6.5% | −20% |

### How the Copula Works

The Gaussian copula preserves each source's Beta marginal distribution exactly while introducing the specified correlation structure. For each Monte Carlo draw:

1. Draw `n_sources` correlated standard normals: `z ~ N(0, R)` via Cholesky decomposition
2. Map each `z_i` to a uniform via the normal CDF: `u_i = Φ(z_i)`
3. Map each `u_i` to a Beta variate via the Beta inverse CDF: `p_i = Beta⁻¹(u_i; αᵢ, βᵢ)`

The marginal distribution of each `p_i` is still exactly `Beta(αᵢ, βᵢ)` — `mu` and `kappa` are respected — but the joint distribution of `(p₁, …, pₙ)` now respects the correlation matrix.

After sampling, the blended combination rule is applied rather than standard Noisy-OR. When all correlations are zero both corrections are no-ops and the result is mathematically identical to `NoisyORModel`.

### Negative Correlation

Setting `rho < 0` means the sources tend to *offset* each other — when one fires at high strength, the other tends to fire at lower strength. When both fire simultaneously, the co-occurrence is somewhat surprising relative to the correlation structure.

In the blended combination rule, `ρ_eff < 0` is **clamped to 0** before blending. This means negative correlations do not produce a "super-additive" combined risk above the Noisy-OR value. Rather, the sampling path still draws negatively correlated Beta values — meaning `p_i` and `p_j` tend to be on opposite sides of their respective distributions when both fire — which the Noisy-OR combination then integrates. In practice, negatively correlated sources that both fire simultaneously tend to produce a combined risk *slightly above* the independent Noisy-OR, because the simultaneous co-firing of anti-correlated sources is itself a stronger-than-expected signal.

```python
# rho = -0.40: Watchlist Hit + Adverse Media both fired
# Independent (rho=0.00): 86.4%
# Negative   (rho=-0.40): 92.5%  ← slightly above independent
# Positive   (rho=+0.75): 74.0%  ← substantially below independent
```

Negative correlations should be used sparingly and only when you have strong empirical evidence that two sources are genuinely inversely related (for example, two competing fraud models that each flag different populations).

### Saving Correlated Configs

```python
model.save_config(
    sources_dict = my_sources,
    output_path  = "noisy_or_config_correlated.json",
    correlations = [
        {"source_a": "Watchlist Hit", "source_b": "Adverse Media", "rho": 0.70},
    ],
    # Pass correlations=[] to remove all correlations from the saved file.
    # If correlations is omitted, existing correlations are preserved.
)
```
...
**Gaussian copula pool sampling (NoisyORModelCopula):**
```
Step 1 — Draw correlated normals via Cholesky:
  z_ind  ~ N(0, I)             shape (n_sources, pool_size)
  z_corr = L @ z_ind           shape (n_sources, pool_size)   [L = chol(R)]

Step 2 — Map to uniforms via normal CDF:
  u_i = Phi(z_corr_i)          element-wise, u_i ∈ (0, 1)

Step 3 — Map to Beta variates via inverse CDF:
  p_i = Beta_ppf(u_i; alpha_i, beta_i)

Step 4 — Blended combination (Noisy-OR ↔ max):
  rho_eff  = mean pairwise correlation across active sources
  noisy_or = 1 - prod(1 - p_i)  for active sources i
  mx       = max(p_i)            for active sources i
  combined = (1 - rho_eff) * noisy_or + rho_eff * mx
```
The marginal distribution of each `p_i` is exactly `Beta(alpha_i, beta_i)` regardless
of the correlation structure. When R = I (all rho = 0), step 1 is a no-op, rho_eff = 0,
and the blended combination reduces to standard Noisy-OR — identical to `NoisyORModel`.

**Blended marginal contributions (NoisyORModelCopula):**
```
c_i = R_blended(all active) − R_blended(all active except i)

where R_blended(S) =
  if |S| == 0:  0
  if |S| == 1:  mu_i  (single source, no blending)
  else:         (1 - rho_eff(S)) * noisy_or(S) + rho_eff(S) * max(mu_j for j in S)
```
This ensures contributions are consistent with the reported risk score. At rho=0
this reduces to the standard Noisy-OR marginal formula. At rho→1 contributions
approach zero for all sources except the strongest, correctly reflecting redundancy.

**kappa_cap rescaling (FeedbackUpdater):**
```
If kappa_new > kappa_cap:
    mu_new    = alpha_new / kappa_new   (unchanged)
    alpha_cap = mu_new * kappa_cap
    beta_cap  = (1 - mu_new) * kappa_cap
    → write (alpha_cap, beta_cap) instead of (alpha_new, beta_new)
```
mu is preserved exactly; kappa is clamped. This keeps the model responsive to
future feedback regardless of how much prior evidence has accumulated.

...
Feedback is supplied as a CSV file with the following columns:
...
| `weight` | No | Fractional weight for this observation (default `1.0`). See [Weighted Observations](#weighted-observations). |

Column names are case-insensitive. Extra columns are ignored. Rows with unrecognised source names or invalid outcomes are skipped with a warning.
...
Each row is one confirmed case. A single file can contain observations for multiple sources. Multiple rows per source are summed before the update is applied.
...
| `min_counts` | `1.0` | Minimum total weighted observations for a source before its parameters are changed. Raise this to require more evidence before trusting an update. |

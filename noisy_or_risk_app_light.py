...
<title>Noisy-OR Risk Explorer (Light)</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=DM+Mono:wght@300;400;500&family=Fraunces:opsz,wght@9..144,400;9..144,600;9..144,700&display=swap" rel="stylesheet">
<style>
:root {
  --bg:      #f5f2ee;
  --surface: #ffffff;
...
.card-title { font-family: 'Fraunces', Georgia, serif; font-weight: 700; font-size: 0.88rem; }
.badge {
  font-size: 0.68rem;
  color: var(--muted);
...
<header>
  ...
  <div style="display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center;gap:6px;margin-left:8px;">
    ...
    <input type="number" id="n-samples" value="20000" min="100" max="500000" step="1000"
      style="width:90px;font-family:'DM Mono','Courier New',monospace;font-size:0.72rem;font-weight:600;color:var(--text);background:rgba(0,0,0,0.04);border:1px solid var(--border);border-radius:6px;padding:4px 8px;outline:none;-webkit-appearance:none;appearance:none;"
      onchange="schedule()" oninput="schedule()">
  </div>
  <button onclick="simulate()" style="margin-left:auto;font-family:'DM Mono','Courier New',monospace;font-size:0.7rem;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;cursor:pointer;border:1px solid var(--border);border-radius:20px;padding:5px 14px;background:rgba(0,0,0,0.04);color:var(--muted);-webkit-transition:all 0.18s ease;transition:all 0.18s ease;" onmouseover="this.style.borderColor='var(--accent)';this.style.color='var(--accent)';" onmouseout="this.style.borderColor='var(--border)';this.style.color='var(--muted)';">&#8635; Re-run</button>
</header>
...
<div class="app">
  ...
  <div class="viz">
    ...
    <div class="stats">
      ...
      <div class="stat">
        <div class="sval" id="v-cert" style="color:#2d8a4e">&#x2014;</div>
        ...
        <div id="cert-bar-bg" style="margin-top:6px;height:3px;background:var(--border);border-radius:2px;overflow:hidden;">
          <div id="cert-bar" style="height:100%;width:0%;background:#2d8a4e;border-radius:2px;-webkit-transition:width 0.4s ease;transition:width 0.4s ease;"></div>
        </div>
      </div>
    </div>
...
function drawMini(i) {
  ...
  var hex = ['#1d6fa4','#c94f1e','#5b3fa8','#b5376e','#3d7a2e','#c27216'][i];
...
function drawHist(hist, mean, p5, p95) {
  ...
  // axes
  ctx.strokeStyle='#c8c0b8'; ctx.lineWidth=1; ctx.setLineDash([]);
  ctx.beginPath(); ctx.moveTo(pL,pT); ctx.lineTo(pL,H-pB); ctx.lineTo(W-pR,H-pB); ctx.stroke();

  // y gridlines
  ctx.setLineDash([3,3]); ctx.strokeStyle='#e8e2da';
  ctx.font='9px "DM Mono","Courier New",monospace';
  ctx.textAlign='right'; ctx.fillStyle='#7a7470';
  for (var i=1; i<=4; i++) {
    var gy = pT + cH*(1-i/4);
  ...
  grd.addColorStop(0, 'rgba(29,111,164,0.75)');
  grd.addColorStop(1, 'rgba(91,63,168,0.15)');
  ctx.fillStyle = grd;
  for (var i=0; i<n; i++) {
  ...
  // x labels
  ctx.textAlign='center'; ctx.fillStyle='#7a7470';
  ctx.font='9px "DM Mono","Courier New",monospace';
  for (var i=0; i<=5; i++) {
  ...
  vl(p5,   '#9a9490', 'P5',   true);
  vl(p95,  '#c94f1e', 'P95',  true);
  vl(mean, '#1d6fa4', 'MEAN', false);
}
...
function apply(d) {
  ...
  var certCol = cert >= 0.80 ? '#2d8a4e' : cert >= 0.50 ? '#b8860b' : '#c94f1e';
  ...
  // ---- Drivers section ----
  var colors = ['#1d6fa4','#c94f1e','#5b3fa8','#b5376e','#3d7a2e','#c27216'];
  var active = [];
  for (var i=0; i<6; i++) {
...
if __name__ == "__main__":
    print("\n  Noisy-OR Risk Explorer (Light Theme)")

...
8. [API Optimizations — Technical Detail](#api-optimizations--technical-detail)
...
10. [Extending the Model](#extending-the-model)
...
11. [Config Versioning](#config-versioning)
...
**`noisy_or_model.py` — Standard model** designed for server scripts, notebooks, and batch analysis jobs. Straightforward to use, easy to inspect, and suitable for workloads that don't require high concurrency.

**`noisy_or_model_api.py` — API-optimized model** built for deployment behind an API endpoint. It inherits all functionality from the standard model and adds a pre-sampled Beta pool, fully vectorized batch scoring, precision tiers, LRU input caching, and zero-downtime hot reload. Approximately 4× faster on batch workloads in benchmarks.
...
```
noisy_or_config.json      <- All configuration: sources, mu/kappa values, version
noisy_or_model.py         <- Standard model (NoisyORModel)
noisy_or_model_api.py     <- API-optimized model (NoisyORModelAPI) — inherits from above
noisy_or_demo.py          <- Standard model demonstration
...
| Tier | Samples | Typical Latency | Recommended Use |
|---|---|---|---|
| `"fast"` | 2,000 | ~2-5 ms | Real-time screening, high-throughput pipelines |
| `"standard"` | 10,000 | ~5-20 ms | Default API response quality |
| `"high"` | 50,000 | ~20-100 ms | Auditable decisions, final risk reports |

The pool is always built at the `"high"` size (50,000 samples). Smaller tiers simply slice a subset of the pool, so there is no cold-start penalty for any tier.
...
**Solution:** At load time, the API model draws a large pool of 50,000 samples per source and stores them as a matrix of shape `(n_sources, 50_000)`. Prediction calls draw `n_samples` random column indices from this pool instead of sampling from scratch.
...
The `n_monte_carlo_samples` config value still sets the default for the standard model. For the API model, named tiers replace raw counts in typical usage:
...
You can still pass an explicit `n_samples=` argument on any call if a tier does not match your needs. The pool is always built at 50,000 (the `"high"` tier size), so all tiers are served from the same pre-computed data with no additional cost.
...
|---|---|
| 2,000 - 5,000 | Fast screening (API `"fast"` tier) |
| 10,000 | Balanced API default (`"standard"` tier) |
| 20,000 | Standard model default |
| 50,000 | High-precision decisions (API `"high"` tier) |
| 100,000+ | Research / validation |

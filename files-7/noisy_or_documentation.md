...
All model settings live in `noisy_or_config.json`. This is the **only file you need to edit** for routine operation.
...
Open `noisy_or_config.json` and add or remove entries from the `"sources"` array. No Python code changes are required.

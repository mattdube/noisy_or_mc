...
| **Validation** | None | Phase 2 runs 4 checks before any production use |
...
**Source sensitivity analysis** shows the average marginal impact of each source matching alone. This is useful for:
- Communicating model behavior to stakeholders
- Identifying sources where the prior might need revisiting
- Detecting if a new source adds meaningful signal
...
**Purpose:** The single source of truth for all scoring math. Both the API and batch pipeline import from here.

**Two public functions:**

```python
# Single record — for API
...
| `impact_width` | `risk_p90 − risk_p10` — how wide the uncertainty is |
...
Only S7 matched (high-confidence source):
  risk_point = 0.3630
  CI 80%     = [0.3261, 0.3982]  width=0.0721  ← narrow = confident

Only S2 matched (low-confidence source):
  risk_point = 0.0690
  CI 80%     = [0.0328, 0.1137]  width=0.0809  ← wider = uncertain
```

Notice that S7 has a narrower CI than S2 despite having a much higher risk — this correctly reflects that S7 has concentration=300 (well-validated) while S2 has concentration=50 (sparse evidence).
...
| `"both"` | Both `.xlsx` and `.csv` *(default)* | Full auditability — keep flat file for data, Excel for review |
...
**CSV output** is a single flat file with all columns in a consistent order: original ID columns → risk score columns → tier labels → impact columns per source.
...
# Both (default)
run_batch("records.csv", "results", output_format="both")
```
...
**Batch run from the pipeline:**
```
250 records | 3 chunks | n_mc=400 | format=BOTH
  Chunk 1/3: records 0–100   (0.01s)
  Chunk 2/3: records 100–200 (0.01s)
  Chunk 3/3: records 200–250 (0.00s)
Excel → results.xlsx
CSV   → results.csv
Complete: 250 records in 0.29s (874 records/sec)
```
...
The `impact_width` field is a genuine 80% credible interval — not a variance approximation. It answers:
...
A record matching only S4 (mean=0.45, concentration=15) will have very wide uncertainty — S4 itself is uncertain. A record matching only S7 (mean=0.35, concentration=300) will be narrow — S7 is well-validated. Same risk tier, different confidence. The original code's `volatility` field didn't capture this correctly.
...
### Scenario: Analyst reviews 10 records flagged by S2
...
# 3 of 10 reviewed records were confirmed risk events
store = bayesian_update_source(store, "S2", n_successes=3, n_trials=10)
...
After:  Beta(5.5, 57.5)  → mean=0.087, std=0.033
```

The mean shifts from 5% toward the observed 30% hit rate, but dampened by the existing prior. Repeated over time, confidence concentrations increase and uncertainty narrows.
...
From pipeline run on standard hardware:

| Operation | Records | n_mc | Time | Rate |
|---|---|---|---|---|
| Single record (API) | 1 | 2,000 | ~4ms | — |
| Batch | 300 | 300 | 0.03s | 10,000/s |
| Full pipeline incl. validation | — | various | 1.4s total | — |

The original PyMC approach required ~30s just for model compilation before any scoring began.
...
# 4. Test scorer
python phase3_scorer.py

# 5. Start API
uvicorn phase4_api:app --host 0.0.0.0 --port 8000

# 6. Run batch
python phase5_batch.py --input your_data.csv
```

### Updating a source's risk estimate
...
**Q: What if source confidence scores are not binary?**
The match matrix supports float values in [0, 1] — a match score of 0.7 means the source contributed `log(1−p_i) × 0.7` to the log-failure sum. This naturally handles fuzzy matching.

**Q: Why does the full-match case score 0.79 rather than near 1.0?**
Because several sources have significant uncertainty. S4 has mean=0.45 with concentration=15 — it frequently draws low values in MC samples, pulling the aggregate down. This is correct behavior: the model is honest about uncertain sources.

**Q: How do I add a new source?**
Add it to `SOURCE_SPECS` in `phase1_priors.py`, re-run Phase 1, and ensure all input data includes the new column. The scorer dimensions are driven by `store["source_order"]` — no hardcoded `8` anywhere in the scoring logic.

**Q: Can I use this without the batch Excel report (just CSV)?**
Yes — if `openpyxl` is not installed, the batch pipeline automatically falls back to CSV output with a warning.
